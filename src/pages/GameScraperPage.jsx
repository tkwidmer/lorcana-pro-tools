...
function makeGenericBookmarkletCode(origin) {
  return `javascript:(function(){ console.log('Bookmarklet init'); var alreadyDone=sessionStorage.getItem('__lorcanaInterceptor'); console.log('Already done:', alreadyDone); if(alreadyDone){sessionStorage.removeItem('__lorcanaInterceptor');} var origDesc=Object.getOwnPropertyDescriptor(WebSocket.prototype,'onmessage'); console.log('Got origDesc:', !!origDesc); var origAddEventListener=WebSocket.prototype.addEventListener; console.log('Got origAddEventListener:', !!origAddEventListener); var done=false; var handler=function(ev){ console.log('Message!', typeof ev.data); if(!done){ try{ var d=JSON.parse(ev.data); console.log('Parsed type:', d.type); if(d.type==='spectator_update'&&d.game){ var match=window.location.href.match(/spectate\\/([a-f0-9-]+)/i); var uuid=match?match[1]:'unknown'; console.log('Found update, opening with uuid:', uuid); done=true; window.open('${origin}/game-scraper?uuid='+uuid+'&data='+encodeURIComponent(JSON.stringify(d.game)),'_blank'); return; } }catch(e){ console.log('Error:', e.message); } } }; Object.defineProperty(WebSocket.prototype,'onmessage',{set:function(h){ console.log('onmessage setter called'); var self=this; var wrapped=function(ev){ handler.call(self,ev); if(h) h.call(self,ev); }; if(origDesc&&origDesc.set){ origDesc.set.call(this,wrapped); } else { this.addEventListener('message',wrapped); } }, get:function(){ return origDesc&&origDesc.get ? origDesc.get.call(this) : this._onmessage; }}); WebSocket.prototype.addEventListener=function(type,listener){ console.log('addEventListener called for:', type); if(type==='message'){ console.log('Wrapping message listener'); var wrapped=function(ev){ handler.call(this,ev); if(listener) listener.call(this,ev); }; return origAddEventListener.call(this,type,wrapped); } return origAddEventListener.call(this,type,listener); }; console.log('Patched WebSocket.prototype'); if(!alreadyDone){ console.log('First run, setting flag and reloading'); sessionStorage.setItem('__lorcanaInterceptor','1'); alert('Bookmarklet activated!\\n\\nReloading the page to apply interception...'); setTimeout(function(){ window.location.reload(); }, 100); } else { console.log('Reload detected, showing waiting alert'); try { alert('Waiting for next game update...\\nThe tool will open automatically in a new tab when the server sends the next update.'); } catch(e) { console.log('Alert blocked:', e.message); } console.log('Alert dismissed'); } })();`
}
