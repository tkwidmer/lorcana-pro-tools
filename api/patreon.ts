...
import { exchangeCodeForTokens, refreshAccessToken, fetchIdentity, selectCampaignMembership } from './_lib/patreonApi.js'
import {
  upsertPatreonLink,
...
// Consolidated Patreon route — folds the four formerly-separate
// patreon-callback.ts / patreon-status.ts / patreon-webhook.ts /
// patreon-reconcile-tick.ts functions into one, dispatched by ?endpoint=,
// to stay under Vercel Hobby's 12-function cap (see CLAUDE.md "Function
// budget"). Each sub-handler below is unchanged from its former standalone
// file — only the routing and raw-body plumbing are new.
//
// bodyParser is off for the whole function because the webhook endpoint
// needs the raw body to verify its HMAC-MD5 signature (see handleWebhook).
// The other endpoints never read req.body, so this is harmless for them —
// req.query (used by callback/reconcile-tick's method check) is populated
// from the URL regardless of body parsing.
export const config = {
...
function callbackRedirectUri(req: VercelRequest): string {
  ...
  return `${proto}://${host}/api/patreon?endpoint=callback`
}
