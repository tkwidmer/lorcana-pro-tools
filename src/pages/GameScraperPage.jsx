...
function makeBookmarkletCode(uuid, origin) {
  return `javascript:(function(){ var orig=EventTarget.prototype.dispatchEvent; var done=false; EventTarget.prototype.dispatchEvent=function(ev){ if(!done){ console.log('Event:', ev.type, ev.constructor.name); if(ev instanceof MessageEvent){ console.log('MessageEvent data:', typeof ev.data, ev.data.substring ? ev.data.substring(0,200) : ev.data); try{ var d=JSON.parse(ev.data); console.log('Parsed JSON:', d.type); if(d.type==='spectator_update'&&d.game){ console.log('Found spectator_update with game!'); done=true; EventTarget.prototype.dispatchEvent=orig; window.open('${origin}/game-scraper?uuid=${uuid}&data='+encodeURIComponent(JSON.stringify(d.game)),'_blank'); } }catch(e){ console.log('Parse error:', e.message); } } } return orig.call(this,ev); }; alert('Waiting for next game update...\\nThe tool will open automatically in a new tab when the server sends the next update (usually within a few seconds).\\n\\nOpen DevTools (F12 > Console) to see debugging info.'); })();`
}
