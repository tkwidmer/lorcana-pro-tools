...
function makeGenericBookmarkletCode(origin) {
  return `javascript:(function(){ console.log('%cLorcana Game Scraper Bookmarklet Activated','color:green;font-weight:bold'); var origAddEventListener=WebSocket.prototype.addEventListener; var done=false; WebSocket.prototype.addEventListener=function(type,listener){ if(type==='message'){ console.log('Intercepting WebSocket message listener'); var self=this; var wrapped=function(ev){ try{ var d=JSON.parse(ev.data); if(d.type==='spectator_update'&&d.game&&!done){ var match=window.location.href.match(/spectate\\/([a-f0-9-]+)/i); var uuid=match?match[1]:'unknown'; console.log('%c✓ Found spectator_update! Opening tool...','color:green;font-weight:bold'); done=true; window.open('${origin}/game-scraper?uuid='+uuid+'&data='+encodeURIComponent(JSON.stringify(d.game)),'_blank'); } }catch(e){} } listener.call(this,ev); }; return origAddEventListener.call(this,type,wrapped); } return origAddEventListener.call(this,type,listener); }; console.log('%cWaiting for next game update... Watch this console for ✓ Found spectator_update!','color:blue'); })();`
}
