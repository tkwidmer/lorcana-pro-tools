...
- `supporter_source = 'metafy'` rows are granted/revoked automatically by the Metafy integration (see below) rather than through `AdminPage`; manual/admin grants (`supporter_source = 'manual'`) are never overwritten by that automation. A handful of legacy `supporter_source = 'patreon'` rows also still exist from a since-removed Patreon integration — see "Metafy Integration" below for their current status.
...
The only automated supporter integration in the app (a prior Patreon integration was removed — see "Legacy Patreon data" below). Backers connect their Metafy account via OAuth from a "Connect Metafy" card on `SettingsPage.jsx`; an active subscription to this app owner's own Metafy community grants `profiles.supporter_tier = 'supporter'` with `supporter_source = 'metafy'`. Docs: [dev.metafy.gg](https://dev.metafy.gg/).
...
**Legacy Patreon data:** an earlier Patreon OAuth integration (`api/patreon.ts` + friends) was removed in favor of Metafy. Its code, UI, and GitHub workflow are gone, but the `patreon_links` table and its `pgcrypto` functions (`supabase/migrations/006_patreon_links.sql`, `007_patreon_links_crypto_functions.sql`) were deliberately left in place rather than dropped, since real linked accounts' data would be destroyed with them. Those rows are now inert — nothing reads or writes them — and any `profiles` row still carrying `supporter_source = 'patreon'` keeps its `supporter_tier` frozen at whatever it was when the integration was removed (nothing will ever revoke it again, since the code path that could is gone).
...
| Supabase `patreon_links` table | row per user (legacy) | Inert leftover from the removed Patreon integration — no code reads or writes it anymore. See "Legacy Patreon data" under Metafy Integration |
