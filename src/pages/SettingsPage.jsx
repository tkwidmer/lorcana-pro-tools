...
async function metafyFetch(method) {
  ...
  const res = await fetch('/api/metafy?endpoint=status', {
    method,
    headers: { Authorization: `Bearer ${accessToken}` },
  })
  if (!res.ok) throw new Error(`Metafy status request failed (${res.status})`)
  return res.json()
}

function useMetafyStatus() {
  const [status, setStatus] = useState(null) // null while loading, else { connected, hasAccess?, lastSyncedAt? }
  const [error, setError] = useState(null)

  async function refresh() {
    try {
      const result = await metafyFetch('GET')
      setStatus(result)
      setError(null)
    } catch (err) {
...
async function connectMetafy() {
  ...
  // has several valid aliases (inkbornforge.com, www.inkbornforge.com,
  // lorcana-pro-tools.vercel.app, the various *-tkwidmers-projects.vercel.app
  // ones, ...), but Metafy's OAuth app only accepts the one exact
  // redirect_uri registered in its dashboard
  // (metafy.gg/account/settings/oauth) — so this must always send that same
  // one regardless of which alias the visitor happens to be on.
  const redirectUri = 'https://www.inkbornforge.com/api/metafy?endpoint=callback'
  const params = new URLSearchParams({
    response_type: 'code',
    client_id: clientId,
    redirect_uri: redirectUri,
    scope: 'profile purchases',
    state: accessToken,
  })
  window.location.href = `https://metafy.gg/auth/authorize?${params.toString()}`
}
...
export function SettingsPage() {
  ...
  async function handleConnectMetafy() {
    ...
    try {
      ...
      setMetafyBanner('error')
      setMetafyBusy(false)
    }
  }

  async function handleDisconnectMetafy() {
    setMetafyBusy(true)
    try {
      await metafyFetch('DELETE')
      await refreshMetafy()
    } finally {
      setMetafyBusy(false)
    }
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 items-start">
        <div className="lg:col-span-1 lg:order-2 flex flex-col gap-6">
          ...
          <div className="border border-gray-200 rounded-lg p-6">
            ...
            {metafyBanner === 'error' && (
              <p className="text-sm text-red-600 font-medium mb-4">Something went wrong connecting Metafy. Please try again.</p>
            )}

            {metafyStatus === null && (
              <p className="text-sm text-gray-400">Loading Metafy status…</p>
            )}
