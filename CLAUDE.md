...
- `SUPABASE_SERVICE_ROLE_KEY` — used by `api/_lib/discordSupabase.ts` for server-side access to the `discord_favorite_players`, `duels_api_tokens`, `patreon_links`, and `metafy_links` tables, bypassing RLS.
- `CRON_SECRET` — shared secret checked by `/api/discord-tournament-tick`, `/api/patreon?endpoint=reconcile-tick`, and `/api/metafy?endpoint=reconcile-tick`; must match the same-named secret in the GitHub repo (Settings → Secrets and variables → Actions) used by `.github/workflows/tournament-tracker-tick.yml`, `.github/workflows/patreon-reconcile-tick.yml`, and `.github/workflows/metafy-reconcile-tick.yml`.
...
- `PATREON_CLIENT_ID` / `PATREON_CLIENT_SECRET` — Patreon OAuth app credentials, used by `/api/patreon?endpoint=callback` and `/api/patreon?endpoint=reconcile-tick` to exchange/refresh tokens. The client ID is not secret (also exposed client-side as `VITE_PATREON_CLIENT_ID`); the client secret is a real secret.
- `PATREON_WEBHOOK_SECRET` — per-webhook HMAC-MD5 secret used by `/api/patreon?endpoint=webhook` to verify `X-Patreon-Signature`, generated when registering the webhook in the Patreon creator dashboard.
...
- `METAFY_API_KEY` — an owner-scoped Metafy API key (scope `community`), used by `/api/metafy?endpoint=reconcile-tick` to list active community subscribers. Unlike Patreon, no per-user OAuth token is ever persisted for Metafy — see "Metafy Integration" below.
...
- `supporter_source = 'patreon'` and `supporter_source = 'metafy'` rows are granted/revoked automatically by the Patreon and Metafy integrations (see below) rather than through `AdminPage`; manual/admin grants (`supporter_source = 'manual'`) are never overwritten by that automation.
...
**Function budget — read before adding a route.** Vercel's Hobby plan caps a deployment at **12 serverless functions**, counted as files in `api/` (`api/_lib/` is excluded). The count is currently **10**. Going over fails the deploy, so the established fix is to consolidate related endpoints into one function dispatched by a query param — see `/api/duels`, `/api/tournament-history`, `/api/patreon`, and `/api/metafy` (`?endpoint=`). (The Patreon integration originally shipped as four separate `patreon-*.ts` files; they were folded into one `api/patreon.ts` specifically to make room for `/api/metafy` under the cap.)
...
| `/api/patreon` | Patreon API | Varies by endpoint — see "Patreon Integration" below | Single consolidated route for the Patreon supporter integration, dispatched by `?endpoint=` — `callback`, `status`, `webhook`, `reconcile-tick`. Folded from four separate `patreon-*.ts` files into one to stay under the function budget. |
...
`api/patreon.ts` is a single consolidated route (four formerly-separate `patreon-*.ts` files folded into one, dispatched by `?endpoint=`, to stay under the function budget above). Three of its endpoints keep `supporter_tier` in sync with Patreon, all funneling through `applyPledgeStateToProfile()` in `api/_lib/patreonSupabase.ts` (the single grant/revoke chokepoint):
- `?endpoint=callback` — the OAuth redirect target. `state` carries the initiating user's Supabase access token (verified server-side via `auth.getUser()`, the same pattern `api/duels-tokens.ts` uses for its Bearer auth) since this is a plain browser redirect with no other way to identify the logged-in user.
- `?endpoint=webhook` — Patreon's webhook for `members:pledge:create/update/delete`, verified via raw-body HMAC-MD5 (`X-Patreon-Signature`) before parsing, mirroring `discord-interactions.ts`'s raw-body-first Ed25519 verification. `bodyParser` is disabled for the whole file because of this endpoint — the other three never read `req.body`, so this is harmless for them.
- `?endpoint=reconcile-tick` — a `CRON_SECRET`-gated daily safety net (`.github/workflows/patreon-reconcile-tick.yml`, same shape as the Discord tournament tick) that re-polls Patreon's identity endpoint for any link that's gone stale, in case a webhook was missed.

`?endpoint=status` is a small authenticated GET/DELETE endpoint so `SettingsPage` can read connection status and disconnect without needing RLS access to `patreon_links`.

**Invariant:** revocation (`applyPledgeStateToProfile(userId, false)`) only ever updates rows where `supporter_source = 'patreon'` — a manually- or admin-granted supporter (via `AdminPage.jsx`) is never touched by a patron cancelling or declining a charge. The Metafy integration below shares this same guard pattern on its own `supporter_source = 'metafy'` value, so the two integrations can't clobber each other's grants — but see that section's note on what a user active on *both* sources still can't get from this single-column design.

### Metafy Integration (`api/metafy.ts`)

Backers connect their Metafy account via OAuth from a "Connect Metafy" card on `SettingsPage.jsx`; an active subscription to this app owner's own Metafy community grants `profiles.supporter_tier = 'supporter'` with `supporter_source = 'metafy'`. Docs: [dev.metafy.gg](https://dev.metafy.gg/).

Architecturally lighter than the Patreon integration: **no OAuth tokens are ever persisted.** The Metafy user ID ↔ Supabase user_id link (plus last-known access state) lives in the `metafy_links` table (`supabase/migrations/010_metafy_links.sql`) — RLS enabled with zero policies, same service-role-only pattern as `patreon_links`. `api/_lib/metafyApi.ts` is the thin client for Metafy's OAuth2 + API v1; `api/_lib/metafySupabase.ts` holds `applyMetafyStateToProfile()`, the single grant/revoke chokepoint (mirroring `patreonSupabase.ts`'s `applyPledgeStateToProfile()`).

`api/metafy.ts` is a single consolidated route, dispatched by `?endpoint=`:
- `?endpoint=callback` — the OAuth redirect target, same `state`-carries-the-Supabase-access-token pattern as `api/patreon.ts`. Uses the user's freshly-issued access token for exactly one call — `GET /v1/me/purchases/communities/{communityId}` (`METAFY_COMMUNITY_ID`) — to grant/revoke immediately on connect, then discards the token entirely; it's never written to `metafy_links`.
- `?endpoint=status` — authenticated GET/DELETE so `SettingsPage` can read connection status and disconnect, same shape as `api/patreon.ts`'s `?endpoint=status`.
- `?endpoint=reconcile-tick` — a `CRON_SECRET`-gated safety net, but unlike Patreon's daily tick this is the **primary** revoke path: Metafy's webhooks are Partner-only ([dev.metafy.gg/api-reference/v1/webhooks](https://dev.metafy.gg/api-reference/v1/webhooks)), so there's no real-time push at all here. It runs every 30 minutes (`.github/workflows/metafy-reconcile-tick.yml`, same cadence as `discord-tournament-tick`), and does a single owner-scoped `GET /v1/me/community/subscribers` call (`METAFY_API_KEY`) rather than re-polling each linked user's own token — that's what makes storing/refreshing per-user tokens unnecessary.

There is deliberately no PKCE code-verifier flow here, even though Metafy's docs recommend one for authorization-code integrations: PKCE protects clients that can't hold a secret, and this integration exchanges the code server-side with `METAFY_CLIENT_SECRET`, the same trust model the existing Patreon flow already relies on without PKCE.

**Invariant:** revocation (`applyMetafyStateToProfile(userId, false)`) only ever updates rows where `supporter_source = 'metafy'` — same guard as the Patreon chokepoint, and manually-/admin-granted/Patreon-granted supporters are never touched by a Metafy subscription lapsing. Note this is a single-column design shared with Patreon/manual/kofi: a user active on *both* Patreon and Metafy still only occupies one `supporter_source` value at a time (whichever last wrote it), so the *other* source's next revoke is a no-op against them rather than actually removing access. This is an accepted limitation carried over from the existing Patreon/manual/kofi setup, not something new introduced here.
...
| Supabase `patreon_links` table | row per user | Patreon user ID ↔ Supabase user_id, pledge status, OAuth tokens encrypted at rest (pgcrypto). Managed via `api/patreon.ts` (`?endpoint=callback`/`webhook`/`status`/`reconcile-tick`) |
