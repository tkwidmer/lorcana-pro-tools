...
function makeBookmarkletCode(uuid, origin) {
  return `javascript:(function(){ var OrigWS=window.WebSocket; var done=false; window.WebSocket=function(...args){ var ws=new OrigWS(...args); Object.defineProperty(ws,'onmessage',{set:function(h){ console.log('WebSocket onmessage handler set'); this._h=function(ev){ console.log('WS message:', typeof ev.data, ev.data.substring ? ev.data.substring(0,100) : ev.data); if(!done){ try{ var d=JSON.parse(ev.data); console.log('Parsed:', d.type); if(d.type==='spectator_update'&&d.game){ console.log('Found it!'); done=true; window.open('${origin}/game-scraper?uuid=${uuid}&data='+encodeURIComponent(JSON.stringify(d.game)),'_blank'); } }catch(e){ console.log('Parse error:', e.message); } } if(h) h.call(this,ev); }; },get:function(){ return this._h; }}); return ws; }; alert('Waiting for next game update...\\nThe tool will open automatically in a new tab when the server sends the next update (usually within a few seconds).\\n\\nOpen DevTools (F12 > Console) to see debugging info.'); })();`
}
