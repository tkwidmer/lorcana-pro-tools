...
function parseGamelog(id, logs, meta = {}) {
  ...
  // Cards that draw when played (card ID → draw count). Used to attribute
  // subsequent CARD_DRAWN events when ABILITY_TRIGGERED isn't fired on-play.
  const ON_PLAY_DRAWS = { '10-66': 2 } // Junior Woodchuck Guidebook

  // Track the last card played by each player for effect attribution
  const lastPlayedByPlayer = { 1: null, 2: null }
  // Pending draw attribution (for on-play draw cards)
  ...
  for (const entry of logs) {
    ...
    for (const card of cardRefs) {
      ...
      if (type === 'CARD_DRAWN') {
        c.drawn++
        // Attribute draw to a pending on-play draw source (e.g. Woodchuck Guidebook)
        ...
        // Start pending draw attribution if this card draws on play
