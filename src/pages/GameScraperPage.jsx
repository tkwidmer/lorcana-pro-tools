...
function makeGenericBookmarkletCode(origin) {
  return `javascript:(function(){ console.log('Bookmarklet init'); var alreadyDone=sessionStorage.getItem('__lorcanaInterceptor'); console.log('Already done:', alreadyDone); if(alreadyDone){sessionStorage.removeItem('__lorcanaInterceptor');} var origDesc=Object.getOwnPropertyDescriptor(WebSocket.prototype,'onmessage'); console.log('Got origDesc:', !!origDesc); var done=false; Object.defineProperty(WebSocket.prototype,'onmessage',{set:function(h){ console.log('onmessage setter called'); var self=this; var wrapped=function(ev){ console.log('Message!', typeof ev.data); if(!done){ try{ var d=JSON.parse(ev.data); console.log('Parsed type:', d.type); if(d.type==='spectator_update'&&d.game){ var match=window.location.href.match(/spectate\\/([a-f0-9-]+)/i); var uuid=match?match[1]:'unknown'; console.log('Found update, opening with uuid:', uuid); done=true; window.open('${origin}/game-scraper?uuid='+uuid+'&data='+encodeURIComponent(JSON.stringify(d.game)),'_blank'); return; } }catch(e){ console.log('Error:', e.message); } } if(h) h.call(self,ev); }; if(origDesc&&origDesc.set){ console.log('Calling original setter'); origDesc.set.call(this,wrapped); } else { console.log('Using addEventListener fallback'); this.addEventListener('message',wrapped); } }, get:function(){ return origDesc&&origDesc.get ? origDesc.get.call(this) : this._onmessage; }}); console.log('Patched WebSocket.prototype.onmessage'); if(!alreadyDone){ console.log('First run, setting flag and reloading'); sessionStorage.setItem('__lorcanaInterceptor','1'); alert('Bookmarklet activated!\\n\\nReloading the page to apply interception...'); setTimeout(function(){ window.location.reload(); }, 100); } else { console.log('Reload detected, showing waiting alert'); alert('Waiting for next game update...\\nThe tool will open automatically in a new tab when the server sends the next update.'); } })();`
}
